...
import it.unina.android.ripper.extractor.ReflectionNoValuesExtractor;
...
public class RipperTestCase extends ActivityInstrumentationTestCase2 {
	...
	public void testApplication() {
		...
		this.extractor = new ReflectionNoValuesExtractor(this.robot);
	...
	private IAnrdoidRipperServiceCallback mCallback = new IAnrdoidRipperServiceCallback.Stub() {
		...
		public void receive(Map message) throws RemoteException {
			...
			Debug.info("Received message: " + msg.getType());
			...
			if (msg.isTypeOf(MessageType.CONFIG_MESSAGE)) {
				...
				Integer widgetIndex = Integer.parseInt(msg.get("widgetIndex"));
				...
				try {
					automation.setInput(widgetId, widgetIndex, widgetName, widgetType, inputType, value);
					Message m = Message.getAckMessage();
					m.addParameter("index", msg.get("index"));
					mService.send(m);
				} catch (Throwable t) {
