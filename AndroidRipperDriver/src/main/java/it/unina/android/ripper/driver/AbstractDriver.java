...
public abstract class AbstractDriver {
    ...
    public void writeGraphFile(Graph graph) {
        String reportXML = "<?xml version=\"1.0\"?><report>\n";

        for (Edge value : graph.getEdges().values()) {
            reportXML += "<activity>\n " +
                    "<source>" + value.getSource().getId() + "</source>\n" +
                    "<target>" + value.getTarget().getId() + "</target>\n" +
                    new XMLRipperOutput().outputTask(value.getTransition()) +
                    "</activity>\n";
        ...
        reportXML += "</report>";
        writeStringToFile(reportXML, XML_OUTPUT_PATH + "graph.xml");
    }

    public void writeDotGraphFile(Graph graph) {
        String reportXML = "digraph G {\n";
        ...
        reportXML += "}";
        writeStringToFile(reportXML, XML_OUTPUT_PATH + "graph.dot");
    }
